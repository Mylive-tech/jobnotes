               ...
               <div class="form-group nopadding search_selection1 col-md-3">			    
                ...
                <div class="form-group col-md-6">
                  ...
                  <label class="checkbox">				  				 
                    <input type="checkbox" <?php if ($objRs->enable_image == 0) echo 'checked';?> class="uniform" onclick="if($(this).is(':checked')) {$('#db_enable_image').val(0);} else {$('#db_enable_image').val(1);}" name="chk" id="chk" value="1">
                    <b>Hide Image on Front Page</b>
                  </label>
                  <input type="hidden" value="<?php if($objRs->id >0) echo ($objRs->enable_image); else echo '1';?>" name="db_enable_image" id="db_enable_image">
                </div>			
                <div class="clearfix"></div> 
                ...
                <div class="form-group col-md-6">
                  <label class="checkbox">				  				 
                    <input type="checkbox" <?php if ($objRs->show_locations_home == 0) echo 'checked';?> class="uniform" onclick="if($(this).is(':checked')) {$('#db_show_locations_home').val(0);} else {$('#db_show_locations_home').val(1);}" name="chk_location" id="chk_location" value="1">
                    <b>Hide Listing on Front Page</b>
                  </label>
                  <input type="hidden" value="<?php if($objRs->id>0) echo ($objRs->show_locations_home); else echo '1';?>" name="db_show_locations_home" id="db_show_locations_home">
                </div>	
...
<div class="crumbs">
    ...
    <div class="row">					
      <div class="col-md-12">						
        ...
        {
			$sdata = $this->modifyContent(TBL_SERVICE);
			if($sdata)
			{
				echo $sdata;
			}
			$objRs = $this->get_admin_serviceListing_Details();
        }
        ?>                                      
			 ...
			 $intI=1;
			  ...
			  while($objRow = $objRs->fetch_object())  // Fetch the result in the object array
			  {
				...
				if($intI++%2==0)  // Condition for alternate rows color
                  ...
                  <a href="<?php echo ISP :: AdminUrl('service/modifyservice/delete/'.$objRow->id.'/status/'.$strStatus);?>" alt="Click to <?php echo ($objRow->status==1)?'Deactivate':'Activate';?>" title="Click to <?php echo ($objRow->status==1)?'Deactivate':'Activate';?>">
